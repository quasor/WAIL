...
; WAIL plugin installation options
Var InstallVST3
Var InstallCLAP
Var VST3Dir
Var CLAPDir
...
; 6. Plugin installation options page
Page custom PluginOptionsPage PluginOptionsLeave
...
; WAIL plugin installation options page
Function PluginOptionsPage
  ; Skip this page in passive/silent mode
  ${IfThen} $PassiveMode = 1 ${|} Abort ${|}

  ; Set default dirs on first visit
  StrCmp $VST3Dir "" 0 +2
  ...
  nsDialogs::Create 1018
  Pop $PluginDialog
  StrCmp $PluginDialog error done

  !insertmacro MUI_HEADER_TEXT "Plugin Installation" "Choose which WAIL plugins to install and where."

  ${NSD_CreateCheckbox} 0 10u 100% 12u "Install VST3 plugins (WAIL Send + Recv)"
  Pop $VST3Check
  ${NSD_SetState} $VST3Check $InstallVST3

  ${NSD_CreateLabel} 12u 28u 80u 10u "VST3 directory:"
  Pop $0
  ${NSD_CreateDirRequest} 12u 40u 200u 13u $VST3Dir
  Pop $VST3DirCtrl
  ${NSD_CreateBrowseButton} 216u 40u 60u 13u "Browse..."
  Pop $0
  ${NSD_OnClick} $0 OnBrowseVST3

  ${NSD_CreateCheckbox} 0 62u 100% 12u "Install CLAP plugins (WAIL Send + Recv)"
  Pop $CLAPCheck
  ${NSD_SetState} $CLAPCheck $InstallCLAP

  ${NSD_CreateLabel} 12u 80u 80u 10u "CLAP directory:"
  Pop $0
  ${NSD_CreateDirRequest} 12u 92u 200u 13u $CLAPDir
  Pop $CLAPDirCtrl
  ${NSD_CreateBrowseButton} 216u 92u 60u 13u "Browse..."
  Pop $0
  ${NSD_OnClick} $0 OnBrowseCLAP

  nsDialogs::Show
  done:
FunctionEnd

Function OnBrowseVST3
  ${NSD_GetText} $VST3DirCtrl $VST3Dir
  nsDialogs::SelectFolderDialog "Select VST3 plugin directory" $VST3Dir
  Pop $0
  StrCmp $0 error +2
    ${NSD_SetText} $VST3DirCtrl $0
FunctionEnd

Function OnBrowseCLAP
  ${NSD_GetText} $CLAPDirCtrl $CLAPDir
  nsDialogs::SelectFolderDialog "Select CLAP plugin directory" $CLAPDir
  Pop $0
  StrCmp $0 error +2
    ${NSD_SetText} $CLAPDirCtrl $0
FunctionEnd

Function PluginOptionsLeave
  ${NSD_GetState} $VST3Check $InstallVST3
  ${NSD_GetText} $VST3DirCtrl $VST3Dir
  ${NSD_GetState} $CLAPCheck $InstallCLAP
  ${NSD_GetText} $CLAPDirCtrl $CLAPDir
FunctionEnd
...
Section Install
  ...
  ; Install WAIL VST3 plugins if selected
  ...
  ; Install WAIL CLAP plugins if selected
...
Section Uninstall
  ...
  ; Remove WAIL VST3 plugins if they were installed by this installer
  ReadRegDWORD $0 HKLM "Software\WAIL\PluginPaths" "InstalledVST3"
  ${If} $0 = 1
    ReadRegStr $1 HKLM "Software\WAIL\PluginPaths" "VST3Dir"
    RMDir /r "$1\wail-plugin-send.vst3"
    RMDir /r "$1\wail-plugin-recv.vst3"
  ${EndIf}

  ; Remove WAIL CLAP plugins if they were installed by this installer
  ...
  ${If} $UpdateMode <> 1
    DeleteRegKey HKLM "Software\WAIL\PluginPaths"
    DeleteRegKey /ifempty HKLM "Software\WAIL"
  ${EndIf}
